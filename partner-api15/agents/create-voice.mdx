...
<ParamField body="general_tools" type="array">
  ...
  Types: `end_call`, `transfer_call`, `custom` (calls your endpoint)
</ParamField>
...
Automatically extract custom data from call transcripts after each call ends. Results are stored in the call history and available via [extraction endpoints](/partner-api/call-history/extractions).

<ParamField body="extraction_fields" type="array">
  List of field names to extract from conversations. Simple string array that auto-generates extraction instructions.
  
  Example: `["credit_score", "loan_amount", "annual_income", "interested", "timeline"]`
  ...
  **How it works:**
  ...
  - Results saved to `extracted_fields` in call history
  - Accessible via [GET /v1/voice/calls/{call_id}/extractions](/partner-api/call-history/extractions)
</ParamField>
...
Extracted data is automatically white-labeled (technology names removed).
</Info>

<ParamField body="post_call_analysis_data" type="array">
  **Advanced:** Define custom fields with specific types and validation. Use `extraction_fields` for simpler configuration.
  ...
  **Number Type** - Extract numeric values:
  ```json
  {
    "type": "number",
    "name": "budget",
    "description": "The customer's stated budget amount"
  }
  ```
...
curl -X POST https://api.teli.ai/v1/agents \
  ...
  -d '{
    ...
    "starting_message": "Hi {{first_name}}, this is Sarah. Do you have a moment to discuss your mortgage needs?",
    "prompt": "You are a mortgage lead qualifier. Ask about their credit score, loan amount, interest rate, and loan type.",
    "organization_id": "1762896364768x389173798861431550",
    "user_id": "1762896366429x599348279945132448",
    "voice_id": "11labs-Rachel",
    "language": "en-US",
    "extraction_fields": ["credit_score", "loan_amount", "interest_rate", "loan_type"]
  }'
```
...
```json 201 Success
{
  ...
  "extraction_fields": ["credit_score", "loan_amount"],
