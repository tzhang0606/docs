...
<CodeGroup>
```bash cURL
curl "https://api.teli.ai/v1/campaigns/camp_abc123/messages?limit=50" \
  -H "X-API-Key: YOUR_API_KEY"
```

```javascript JavaScript
const response = await fetch(
  'https://api.teli.ai/v1/campaigns/camp_abc123/messages?limit=50',
  {
    headers: {
...
response = requests.get(
    'https://api.teli.ai/v1/campaigns/camp_abc123/messages',
    params={'limit': 50},
...
async function pollMessages(campaignId) {
  const url = lastTimestamp 
    ? `https://api.teli.ai/v1/campaigns/${campaignId}/messages?since=${lastTimestamp}`
    : `https://api.teli.ai/v1/campaigns/${campaignId}/messages`;
