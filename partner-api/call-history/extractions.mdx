...
<ResponseField name="lead_outcome" type="string">
  Lead classification result: `qualified`, `dnc`, `pending`, `transferred`, etc.
</ResponseField>
...
<ResponseField name="do_not_message" type="boolean">
  Whether customer opted out of messages
</ResponseField>

<ResponseField name="call_transferred" type="boolean">
  Whether call was transferred to a human
</ResponseField>
...
<Check>
  **Use webhooks** - Listen for `call_analyzed` webhook to know when extraction is complete
</Check>
...
The `call_analyzed` webhook event includes extraction data:
...
Use this webhook to trigger downstream actions when extraction completes.
