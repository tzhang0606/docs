...
<ResponseField name="campaigns" type="array">
  ...
  <Expandable title="Voice Campaign Object">
    ...
    <ResponseField name="campaign_begin_time" type="timestamp">
      When campaign calling started. **Currently returned as an empty string** in list responses (not stored in the list source table).
    </ResponseField>
    
    <ResponseField name="campaign_end_time" type="timestamp">
      When campaign calling ended (or scheduled to end). **Currently returned as an empty string** in list responses (not stored in the list source table).
    </ResponseField>
...
```bash cURL
curl -X GET "https://api.teli.ai/v1/voice/campaigns?organization_id=1762896364768x389173798861431550&is_admin=false" \
