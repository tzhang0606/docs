...
- Timezone and quiet-hours checks are bypassed for human-sent messages
- **Do-Not-Call (DNC) is still enforced.** If the contact is on the organization's DNC list, the send is blocked with a `409` response (see Error Responses). Remove them with [Remove from DNC](/partner-api/dnc/remove) first.
