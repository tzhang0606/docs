...
const response = await fetch(`https://api.teli.ai/v1/organizations/${orgId}`, {
...
<ResponseField name="sms_registration_status" type="string">
  10DLC registration status (e.g. `not_started`, `pending`, `approved`)
</ResponseField>
...
<ResponseField name="total_sms_inbound" type="number">
  Total SMS received (all-time)
</ResponseField>
