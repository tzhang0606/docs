...
curl "https://api.teli.ai/v1/campaigns/camp_abc123/contacts?user_id=admin_001&is_admin=true" \
  -H "X-API-Key: YOUR_API_KEY"
```

```bash cURL (user - assigned only)
curl "https://api.teli.ai/v1/campaigns/camp_abc123/contacts?user_id=user_xyz789&is_admin=false" \
  -H "X-API-Key: YOUR_API_KEY"
```

```javascript JavaScript
const response = await fetch(
  'https://api.teli.ai/v1/campaigns/camp_abc123/contacts?user_id=user_xyz789&is_admin=false',
  {
    headers: {
...
response = requests.get(
    'https://api.teli.ai/v1/campaigns/camp_abc123/contacts',
