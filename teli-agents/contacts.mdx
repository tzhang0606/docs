...
When running a campaign, you will almost always engage with contact groups. Contact groups are a collection of contacts that you will want Teli's AI Agents to call. Teli will accept CSV files and automatically create mapping suggestions to parse your data. You can map fields such as first name, last name, birthday, and more fields (see section of dynamic variables for the full list). **For each contact group, you are limited to 10,000 contacts.** Please note that you can still run multiple campaigns concurrently with multiple contact groups.
