...
- The same email can exist across multiple organizations, each with its own `unique_id` and permissions
- Usage metrics are tracked per user for billing
- Admin permission grants organization-wide visibility
- When referencing a user in API calls, always use the `unique_id`, not the email address
